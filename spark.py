...
if __name__ == '__main__':
    ...
    plt.xlabel("Les mois")
    plt.ylabel("Nb H/F")
    plt.title("comparaison  entre le nombre (en %) de F et H")
    plt.legend(facecolor="grey")
    plt.show() 


    #Etude age : qui d'entre moins de 30ans ou plus d e30ans se font le plus viré
    plt.bar(x1,sup,label="age > 30",width=.5)
    plt.bar(x1,inf,label="age < 30",width=.4)
    plt.xlabel("Les mois")
    plt.ylabel("Nb H/F")
    plt.title("comparaison par age")
    plt.legend(facecolor="grey")
    plt.show() 
    ...
    plt.title("raisons pour les quels un bac+12 sont en chomages")
    plt.show()
